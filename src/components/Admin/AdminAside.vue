<template>
    <aside class="z-20 hidden pt-5 w-64 overflow-y-auto px-6 bg-white md:block flex-shrink-0 duration-200 "
        ...
        <div class="space-y-2 pb-2">
            <a
                ...
                <svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision"
                    ...
                    clip-rule="evenodd" viewBox="0 0 496 512.24" class="w-6">
            ...
            <AdminSidebarItem v-for="(item, index) in items" :key="index" :path="item.path" :icon="item.icon"
                :text="item.text" :children="item.children" @press="menuOpen = false" />
        </div>
    </aside>

    <div class="fixed w-full h-full bg-[#0000003a] z-10" v-if="menuOpen" @click="menuOpen = false"></div>
</template>
