<template>
    <div class="min-w-[30rem] p-4">
        ...
        <div
            ...
            @dragenter.prevent
            @dragover.prevent>
            <div class="h-[11.5rem] flex items-center justify-center font-bold text-gray-400 select-none" v-if="data.length == 0">
                Drag here a card
            </div>
            <slot></slot>
        </div>
        <button
            type="button"
            class="text-gray-400 w-full font-bold outline-dashed outline-2  outline-gray-300 rounded-lg py-2 " v-if="buttonFlag">
            + Add another Task
        </button>
    </div>
</template>
