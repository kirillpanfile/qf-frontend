...
export const useAdminStore = defineStore("adminStore", {
    ...
    actions: {
        async authAdmin(user) {
            await errorHandler(
                async function () {
                    this.user = await Window.$http.post(process.env.VUE_APP_SIGNIN_ADMIN, user)
                    this.isAuth = true
                }.bind(this),
        ...
        async authRemeber() {
            await errorHandler(
                async function () {
                    this.user = await Window.$http.get(process.env.VUE_APP_SIGNIN_REMEMBER_ADMIN)
        ...
        async loadUsers() {
            await errorHandler(
                async function () {
                    this.users = await Window.$http.get(process.env.VUE_APP_GET_ALL_USERS + "?page=" + this.currentPage)
                }.bind(this),
        ...
        async loadNewUsers() {
            await errorHandler(
                async function () {
                    this.newUsers = await Window.$http.get(process.env.VUE_APP_GET_NEW_USERS)
                }.bind(this),
        ...
        async searchUser(user) {
            await errorHandler(
                async function () {
                    const res = await Window.$http.get(process.env.VUE_APP_SEARCH_USERS + user)
        ...
        async editUser({ username, email, roles, id }) {
            await errorHandler(
                async function () {
                    const updatedUser = await Window.$http.put(process.env.VUE_APP_UPDATE_USER + id, {
                        username,
                        email,
                        roles,
                    })
        ...
        async getAdmins() {
            await errorHandler(
                async function () {
                    this.admins = await Window.$http.get(process.env.VUE_APP_GET_ALL_ADMINS)
                }.bind(this),
